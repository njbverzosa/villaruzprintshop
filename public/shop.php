...
// Get cart item count for the users menu - FIXED: Use $accNumber from session
$cartCountStmt = $pdo->prepare("SELECT SUM(pieces) as total_items FROM cart WHERE acc_number = ?");
$cartCountStmt->execute([$accNumber]); // ← USE $accNumber from session
$cartCountResult = $cartCountStmt->fetch(PDO::FETCH_ASSOC);
$cartTotalItems = intval($cartCountResult['total_items'] ?? 0);

// Get product_name from URL parameter, set search
$productNameFromUrl = isset($_GET['product_name']) ? trim($_GET['product_name']) : '';
...
<a href="closed.php" class="nav-item" onclick="return confirm('Are you sure you want to logout?');">
    ...
    <script>
        ...
        const accNum = '<?php echo htmlspecialchars($accNumber); ?>'; // ← FIXED: Use $accNumber from session
        const productNameFromUrl = '<?php echo addslashes($productNameFromUrl); ?>';
...
// ✅ Check if user is logged in - ONLY check user_id and user_type
...
if ($storedDate !== $currentDateTime) {
    ...
    $updateStmt->execute([$currentDateTime, $user['id']]);
...
<body>
    ...
    <script>
        ...
        // ============================================================
        // SEARCH WITH CLEAR BUTTON - FIXED
        // ============================================================
        const searchInput = document.getElementById('liveSearchInput');
        const clearBtn = document.getElementById('clearSearchBtn');
        const searchIcon = document.getElementById('searchIcon');
        ...
        // Show/hide clear button on input and trigger search
        searchInput.addEventListener('input', function() {
            toggleClearButton();
            if (searchTimeout) {
                clearTimeout(searchTimeout);
        ...
        // AUTO-SEARCH FROM URL PARAMETER
        // ============================================================
        // If product_name exists in URL, trigger search after page load
        if (productNameFromUrl && productNameFromUrl.trim() !== '') {
            // Wait for DOM to be fully loaded
            setTimeout(function() {
                // Trigger the search
                filterProducts();
                // Make sure the clear button shows
                toggleClearButton();
...
>>>>>>> a8e835a22c0311bd16b6df3288ded96fea8a6566
