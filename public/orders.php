...
$cartCountStmt->execute([$accNumber]); // ← FIXED: Use $accNumber
...
$stmt = $pdo->prepare("SELECT delivery_number, delivery_date, total_amount, charge, status, ordered_by, delivery_address, date_time_sold FROM for_deliveries WHERE acc_number = ? AND status = 'PENDING' ORDER BY id DESC");
$stmt->execute([$accNumber]); // ← FIXED: Use $accNumber instead of $userAccNumber
