...
if ($action === 'add_product') {
    ...
    try {
        ...
        if ($result) {
            ...
            // ==============================================
            // 4. INSERT INTO LOGS WITH PROPER NAME
            // ==============================================
            $logDetails = "Added new product: {$productName} | Unit: {$unit} | Quantity: {$quantity} | Price: ₱{$sellingPrice} | Description: " . ($description ?: 'N/A');
            
            $logStmt = $pdo->prepare("INSERT INTO logs (name, action, details, created_at) VALUES (?, ?, ?, ?)");
            $logStmt->execute([
                $firstName,           // Now this will be "Norlie" or whatever the user's first name is
                "Added New Product",
                $logDetails,
                $last_restocked
            ]);
