<?php
// Customer_API/chat.php - Backend for Live Chat
...
try {
    switch ($action) {
        ...
        case 'get_conversations':
            ...
            $stmt = $pdo->prepare("
                SELECT 
                    ...
                    (SELECT message FROM chat_conversation WHERE acc_number = c.acc_number OR receiver_acc = c.acc_number ORDER BY created_at DESC LIMIT 1) as last_message,
                    (SELECT DATE_FORMAT(created_at, '%b %d, %Y %h:%i %p') FROM chat_conversation WHERE acc_number = c.acc_number OR receiver_acc = c.acc_number ORDER BY created_at DESC LIMIT 1) as last_message_time,
                    (SELECT COUNT(*) FROM chat_conversation WHERE receiver_acc = c.acc_number AND status = 0) as unread_count
                FROM chat_conversation c
                LEFT JOIN chat_account ca ON c.acc_number = ca.acc_number
                WHERE c.receiver_acc = ? OR c.acc_number = ?
                GROUP BY c.acc_number
        ...
        case 'toggle_block':
            ...
            $blockStatus = intval($_POST['block_status'] ?? 0); // 0=block, 1=unblock
