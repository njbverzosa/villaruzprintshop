...
if ($userRole === 'Investor') {
    ...
    $stmt = $pdo->prepare("
        SELECT id, acc_number, f_name, email, phone_number, user_name, authorize_access
        FROM admins
        WHERE id = ?
    ");
    $stmt->execute([$userId]);
...
<body>
    ...
    <script>
        ...
        document.getElementById('productForm').addEventListener('submit', async function (e) {
            ...
            // ✅ Confirm CSRF token is included before sending
            ...
            try {
                var res = await fetch('../API/add_product.php', {
                    method: 'POST',
                    body: formData,
                    credentials: 'same-origin'   // ✅ ensure cookies (PHPSESSID) are sent
                });
